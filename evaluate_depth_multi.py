...
#     if args.backbone == "mpvit_xs":
#         depth_encoder = networks.mpvit.mpvit_xsmall()
#         num_ch_enc = np.array([64, 128, 192, 256, 256])
#         depth_decoder = networks.DepthDecoder_Mix(num_ch_enc)

#     depth_encoder.load_state_dict({k: v for k, v in encoder_dict.items() if k in depth_encoder.state_dict()})
...
def load_model(args):
    ...
    depth_encoder = load_weights(depth_encoder, encoder_dict)
    depth_decoder = load_weights(depth_decoder, decoder_dict)
...
def test_cityscapes(args, dataloader, depth_encoder, depth_decoder):
    ...
    # gt_path = os.path.join("/home/jinfengliu/monodepth2-ddp", "splits", "cityscapes", "gt_depths")
    pred_disps = []
    for data in dataloader:
        input_color = data[("color", 0, 0)].cuda()
        if args.post_process:
