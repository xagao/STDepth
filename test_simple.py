...
def test_simple(args):
    ...
    model_path = os.path.join(args.pretrained_path)
    ...
    encoder = networks.mpvit.mpvit_xsmall()
    ...
    depth_decoder = networks.DepthDecoder_Mix_ablation(
        num_ch_enc=num_ch_enc, scales=range(4))

    loaded_dict = torch.load(depth_decoder_path, map_location=device)
    depth_decoder.load_state_dict(loaded_dict,False)
    ...
    with torch.no_grad():
        for idx, image_path in enumerate(paths):
            ...
            # 计算 FLOPS
            flops, params, flops_e, params_e, flops_d, params_d = profile_once(encoder, depth_decoder, input_image)
            ...
            name_dest_im = os.path.join(output_directory, "{}_xs.jpeg".format(output_name))
